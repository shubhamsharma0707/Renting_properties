...
export class MapView {
  ...
  bindEvents() {
    ...
    // ⚡ Bolt: Debounce the heavy refreshCards operation to prevent unnecessary re-renders while dragging the slider
    const debouncedRefresh = debounce(() => this.refreshCards(), 150);

    budgetSlider?.addEventListener('input', e => {
      this.params.budget = parseInt(e.target.value);

      // Update UI elements instantly for responsive feel
      budgetLabel.textContent = formatINR(this.params.budget);
      this.updateSliderGradient(budgetSlider);

      // Debounce the heavy DOM re-rendering operations
      debouncedRefresh();
    });
    this.updateSliderGradient(budgetSlider);
